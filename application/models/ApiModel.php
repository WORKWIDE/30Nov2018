...
//        }
        ...
        foreach ($result as $r) {
            ...
            if (base64_encode($r['option_type']) == "U0VMRUNU" && strtolower($r['ent_name'])=='nashua') {
                $data[$i]['type_values'] = $this->selectOptionValueOffline($r['id'], $product_line, $r['depondon']);
            } else {
    ...
    public function updateTaskScreen($taskid = NULL, $cat_id = NULL) {
        ...
        foreach ($result as $r) { 
            ...
            $data[$i]['type_limit'] = $r['type_limit'];
            $data[$i]['required_status'] = $r['required_status'];
            if (base64_encode($r['option_type']) == "U0VMRUNU" && strtolower($r['ent_name'])=='nashua'&&($r['id']==1||$r['id']==2)) {
                $data[$i]['type_values'] = $this->selectOptionValue($r['id'], $product_line, $r['depondon']);
            } else {
                $data[$i]['type_values'] = json_decode($r['type_values']);
            }
            //$data[$i]['type_values'] = json_decode($r['type_values']);
...
/* old code
        $this->db->select('option_value');
*/
       // echo $id; echo "   search ".$search; echo "    depondid  ".$depondid; exit();
// new code added by dev.com
	 $this->db->select('DISTINCT(option_value)');
// end new code
        $this->db->from(QM_SELECT_VALUE);
