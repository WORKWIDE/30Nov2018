...
//                    alert('Something went wrong! Please try again!');
                    ...
                    var asset_html = "<tr><td>" + asset_display_name + "</td><td>" + asset_type + "</td><td>" + asset_description + "</td><td><span class='removeasset' data-asset='" + res.id + "'>x</span></td></tr>"
...
//            $('#category' + categories[j].task_type).empty().append('<option value="">Select</option>');
        ...
        htm += '></td>';
        htm += '<td><span class="removecategory" data-category_id ="' + categories[j].id + '">x</span></td></tr>';
...
function showCreate_Api_Yes_list() {
    ...
    for (var i = 0; i < create_yes_list.length; i++) {
        var htm = '';
        var html = "<div class='input-group'><div class='col-md-2'><input type='text' value='" + create_yes_list[i].API_Field + "' class='form-control' id='map_field'></div><div class='col-md-4'><input type='text' value='" + create_yes_list[i].End_Point +
                "' class='form-control' id='map_field'></div><div class='col-md-2'><span  id=remove_ele_id" + create_yes_list[i].API_Mapping_Id + " onclick='remove_create_yes(" + create_yes_list[i].API_Mapping_Id +
                ")' style='margin-top: 11px;' class='glyphicon glyphicon-remove'></span></div></div>";
        $('#append_create_yes_data' + create_yes_list[i].API_Task_Type_id).append(html);
    }
}
...
function showAssets() {
    for (var i = 0; i < assets.length; i++) {
        var htm = '';
        htm += "<tr><td>" + assets[i].display_name + "</td><td>" + assets[i].type + "</td><td>" + assets[i].description + "</td><td><span class='removeasset' data-asset='" + assets[i].id + "'>x</span></td></tr>";
...
function selectedOptions(id, values) {
    ...
    $("input[name=asset_api_" + id + "][value=" + values.assets + "]").attr('checked', 'checked');
