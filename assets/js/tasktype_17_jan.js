...
function submitIntegrateData(name, id) {
    ...
    if (error == 0) {
        ...
        $.ajax({
            ...
            type: 'post',
            dataType: 'json',
            data: {ent_id: ent_id, task_type: id, label: label, type: type, limit: limit, category: category, required: required, value: JSON.stringify(tmp)},
            success: function (res) {
                $('#divLoading').removeClass('show');
                if (res.success) {
                    var htm = '<tr><td>' + label + '</td><td>' + type + '</td><td>' + limit + '</td><td>' + category_txt + '<td>' + tmp.toString() + '</td></td><td></td><td><span class="glyphicon glyphicon-remove removeupdatedata" data-id ="' + res.id + '"></span></td><td><input id="map_data_value_' + res.id + '" type="text" value=""></td><td><span class="fa fa-archive" aria-hidden="true" onclick="updateMapData(' + res.id + ')"></span></td></tr>';
...
function showUpdateData() {
    for (var i = 0; i < update_integrateData.length; i++) {
        var htm = '<tr><td>' + update_integrateData[i].label + '</td><td>' + update_integrateData[i].option_type + '</td><td>' + update_integrateData[i].type_limit + '</td><td>' + update_integrateData[i].category + '<td>' + update_integrateData[i].type_values + '</td></td><td>' + update_integrateData[i].endpoint + '</td><td><span class="glyphicon glyphicon-remove removeupdatedata" data-id ="' + update_integrateData[i].id + '"></span></td><td><input type="text" id="map_data_value_' + update_integrateData[i].id + '" value="' + update_integrateData[i].map_data + '"></td><td><span class="fa fa-archive" aria-hidden="true" onclick="updateMapData(' + update_integrateData[i].id + ')"></span></td></tr>';
