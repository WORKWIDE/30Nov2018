...
//    clone_div.find('#add_update_data').attr({id: "add_update_data_" + id, onclick: "add_update_data(" + id + ")"});
                ...
                if (data == 'success') {
                    ...
                    for (var j = 0; j < create_no.length; j++) {
                        if (attr_id.indexOf(create_no[j].Task_type_ID) == -1) {
                            $("#create_map_fields" + create_no[j].Task_type_ID).html('');
                            $("#update_map_fields" + create_no[j].Task_type_ID).html('');
                            $("#assets_map_fields" + create_no[j].Task_type_ID).html('');
                            $("#onhold_map_fields" + create_no[j].Task_type_ID).html('');
                            $("#reject_map_fields" + create_no[j].Task_type_ID).html('');
...
/*
 * 
 * refer showApiData[customised function] function
 function showCreate_Api_Yes() {
 
 var html = "";
 for (var i = 0; i < create_yes.length; i++) {
 document.getElementById("create_method_" + create_yes[i].qm_task_type_id).value = create_yes[i].API_Method_Name;
 document.getElementById("create_endpoint_" + create_yes[i].qm_task_type_id).value = create_yes[i].API_End_point;
 document.getElementById("create_username_" + create_yes[i].qm_task_type_id).value = create_yes[i].API_User_name;
 document.getElementById("create_password_" + create_yes[i].qm_task_type_id).value = create_yes[i].API_Password;
 document.getElementById("create_xml_file_" + create_yes[i].qm_task_type_id).value = create_yes[i].API_XML_File;
 document.getElementById("create_api_key_" + create_yes[i].qm_task_type_id).value = create_yes[i].API_Key;
 document.getElementById("create_api_settings_id_" + create_yes[i].qm_task_type_id).value = create_yes[i].API_Settings_ID;
 
 if (create_yes[i].API_Method_Name != "") {
 //            $("#create_yes_save_data" + create_yes[i].API_Task_Type_id).hide();
 $("#add_create_api_" + create_yes[i].qm_task_type_id).hide();
 }
 }
 }
 */
...
/*
 * 
 * refer showMapFields [customised function] function
 function showCreate_Api_Yes_list() {
 for (var i = 0; i < create_yes_list.length; i++) {
 var html = '';
 html = "<tr id=remove_map_fields" + create_yes_list[i].API_Mapping_Id + "><td>" + create_yes_list[i].API_Field + "</td><td>" + create_yes_list[i].End_Point + "</td><td><span id=remove_ele_id" + create_yes_list[i].API_Mapping_Id + " data-map_id='" + create_yes_list[i].API_Mapping_Id +
...
function showMapFields(type, data) {
    for (var i = 0; i < data.length; i++) {
        var html = '';
        html = "<tr id=remove_map_fields" + data[i].API_Mapping_Id + "><td>" + data[i].API_Field + "</td><td>" + data[i].End_Point + "</td><td><span id=remove_ele_id" + data[i].API_Mapping_Id + " data-map_id='" + data[i].API_Mapping_Id +
                "' style='margin-top: 11px;' class='glyphicon glyphicon-remove remove_map_fields'></span></td></tr>";
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res.success) {
                    ...
                    var html = "<tr id=remove_map_fields" + res.API_Mapping_Id + "><td>" + res.API_Field + "</td><td>" + res.End_Point + "</td><td><span id=remove_ele_id" + res.API_Mapping_Id + " data-map_id='" + res.API_Mapping_Id +
                            "'  data-type='" + type + "' style='margin-top: 11px;' class='glyphicon glyphicon-remove remove_map_fields'></span></td></tr>";
...
//        var api = $("input[name=" + type + "_api_" + id + "]:checked").val();
        var api = 1;
        var ent_id = $('#ent_id').val();
        var tab_id = 0;
